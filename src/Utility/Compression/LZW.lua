...
local function compress(text)
	local dict = copy(dictionary)
	local key, sequence, size = "", {}, #dict
	local width, spans, span = 1, {}, 0

	local function listkey(k)
		local value = tobase93(dict[k])
		if #value > width then
...
local function decompress(text)
	...
	for width = 1, #groups do
		for value in groups[width]:gmatch(('.'):rep(width)) do
			local entry = dict[tobase10(value)]
			if previous then
				if entry then
					sequence[#sequence+1] = entry
					dict[#dict+1] = previous..entry:sub(1, 1)
				else
